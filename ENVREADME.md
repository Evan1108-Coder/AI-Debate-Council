...
| `OPENROUTER_API_KEY` | No | blank | Optional key used only for live pricing lookups against OpenRouter's model catalog. This does not route debates through OpenRouter and does not unlock models by itself. If left blank, the app still tries public pricing lookup and falls back to the local price table when unavailable. |
